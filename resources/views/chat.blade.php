    ...
    <div class="flex p-4 {{ $loop->last ? '' : 'border-b border-b-gray-400' }} ">
        <div class="mr-2 flex-shrink-0">
            <a href="{{ $chat->user->path() }}">
                <img src="{!! URL::asset('avatars/'. $chat->user->id .'/'. str_replace('http://127.0.0.1:8000/', '', $chat->user->avatar)) !!}" alt="{{ $chat->user->name }}'s avatar" class="rounded-full mr-2" style="width:50px; height:50px;">
            </a>
        </div>
