<div class="flex">
    <form method="POST"
          action="/chats/{{ $chat->id }}/like"
    >
        @csrf

        <div class="items-center mr-4 {{ $chat->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
        <button type="submit"
                    class="flex text-xs"
            >
    ...
    >
        ...
        <div class="items-center {{ $chat->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
        <button type="submit"
                    class="flex text-xs"
            >
            <svg viewBox="0 0 20 20"
